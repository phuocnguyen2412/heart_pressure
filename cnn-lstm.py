...
class ConvNet(nn.Module):
    ...
    def forward(self, x):
        ...
        print("Shape after adaptive pooling:", out.shape)  # Debugging line

        # LSTM expects (batch, seq_len, input_size)
        out = out.permute(0, 2, 1)    # (batch_size, 4, 256)
        print("Shape before LSTM:", out.shape)  # Debugging line
...
if __name__ == '__main__':
    ...
    train_dataloader = DataLoader(train_dataset_sbp, batch_size=batch_size, shuffle=True, num_workers=4)
    val_dataloader = DataLoader(val_dataset_sbp, batch_size=batch_size, shuffle=False, num_workers=4)
